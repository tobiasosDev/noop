...
struct SmartAlarmView: View {
    ...
    /// Presents the guided connect→write→confirm arm sheet after the user enables the strap alarm (fork:
    /// the arm is not fire-and-forget — we show the live arming progress and read-back confirmation).
        ...
        StrandCard(padding: 20, tint: behavior.smartAlarmEnabled ? StrandPalette.accent : nil) {
            VStack(alignment: .leading, spacing: 16) {
                ...
                if behavior.smartAlarmEnabled {
                    ...
                    // Live read-back confirmation (fork): shows "syncing…", "confirmed on your strap", or
                    // "didn't store — phone backup is set" from the arm coordinator's GET_ALARM_TIME check.
                    Divider().overlay(StrandPalette.hairline)
                    AlarmStatusRow()
                        .frame(maxWidth: .infinity, alignment: .leading)
                }
            }
            // Fork wiring (#535/#766): the strap alarm is armed through the guided coordinator
            // (connect → write → confirm) with a phone-notification backup, NOT fire-and-forget. UI
            // triggers route through armCoordinator.arm(); AppModel's background re-arm paths still call
            // applySmartAlarm() directly (unchanged), so the daily/reconnect re-arm is untouched.
            ...
            // Time change re-arms silently (no sheet — a compact picker commits on every tick and would
            // pop the dialog repeatedly); the AlarmStatusRow above surfaces the re-confirm inline instead.
            .onChangeCompat(of: behavior.smartAlarmMinutes) { _ in
            ...
            .sheet(isPresented: $showArmSheet) {
                ArmAlarmSheet(
                    ...
                    onRetry: { model.armCoordinator.arm(wakeDate: nextSmartWake) }
                )
            }
        }
    }
    ...
    /// `AppModel.applySmartAlarm`). All UI arm triggers use this so enable / time-change / weekday agree on
    /// one instant. `nextSmartAlarmDate` is a pure static func; the `??` covers its only nil case (a
    /// corrupted weekday set with no valid day) by falling back to the weekday-blind next occurrence.
...
/// Live read-back status of the strap firmware alarm (#766, moved here from Automations with the alarm UI).
/// Reflects `LiveState.alarmArmConfirmed`: nil = arming/syncing, true = the strap confirmed it stored the
/// wake epoch, false = the strap didn't store it (so the phone backup is the fallback). Making a silent
/// non-store VISIBLE is the whole point of the guided arm.
struct AlarmStatusRow: View {
