...
final class SleepStagerTests: XCTestCase {
    ...
    // MARK: - #353 night continuation (late sleepers / shift workers)

    /// A real overnight sleep whose TAIL runs past the daytime-band start (a brief morning stir
    /// then back to sleep) must keep its late wake. The tail centers in the daytime band and fails
    /// the stricter daytime resting bar, so without the overnight-continuation exemption it was
    /// rejected and the wake truncated to ~10:00 ("woke at noon" bug). Because the tail directly
    /// continues a chain that began overnight (gap ≤ 90 min), it is kept — wake reaches ~12:40.
